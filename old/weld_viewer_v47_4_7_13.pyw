...
class WeldViewerApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        left = ttk.Frame(main, width=500);  main.add(left, weight=0)
    ...
    def _build_right(self, p):
        ...
        # *** v4.7.13 *** Disabilita drag-reorder dei tab
        nb.bind("<ButtonPress-1>",   lambda e: "break" if nb.identify(e.x, e.y) == "label" else None)
        nb.unbind_class("TNotebook", "<ButtonPress-1>")
        nb.unbind_class("TNotebook", "<ButtonRelease-1>")
        nb.unbind_class("TNotebook", "<B1-Motion>")
    ...
    def _build_sim_tab(self, parent):
        ...
        lf_outer = ttk.Frame(pane, width=500);  pane.add(lf_outer, weight=0)
        lf_outer.pack_propagate(False)          # *** impedisce resize da contenuto ***
        
        # Canvas scrollabile
        sim_canvas = tk.Canvas(lf_outer, bg=DARK_BG, highlightthickness=0, width=480)
    ...
    def _build_plc_dbreader(self, parent):
        ...
        pane = ttk.PanedWindow(parent, orient="horizontal")
        pane.pack(fill="both", expand=True)
        left = ttk.Frame(pane, width=500);  pane.add(left, weight=0)
    ...
    def _build_plc_autoexport(self, parent):
        ...
        pane = ttk.PanedWindow(parent, orient="horizontal")
        pane.pack(fill="both", expand=True)
        left_outer = ttk.Frame(pane, width=500); pane.add(left_outer, weight=0)
    ...
    def _build_sqlite_import_tab(self, parent):
        ...
        left  = ttk.Frame(main, width=500); main.add(left,  weight=0); left.pack_propagate(False)
        right = ttk.Frame(main);            main.add(right, weight=1)
        # Sorgente
