...
def _kill_process_tree():
    """Termina tutti i processi figli e l'intero process group al momento
    della chiusura del SW — garantisce zero processi zombie nel Task Manager."""
    pid = os.getpid()
    try:
        if sys.platform == "win32":
            # Windows: taskkill uccide l'intero albero ricorsivamente
            import subprocess
            subprocess.call(
                ["taskkill", "/F", "/T", "/PID", str(pid)],
                stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        else:
            # Linux/Mac: kill del process group
            pgid = os.getpgid(pid)
            os.killpg(pgid, signal.SIGKILL)
    except Exception:
        # Fallback: kill diretto del pid principale
...
OPCUA_LIB = None
try:
    from opcua import Client as OpcClient, ua as opcua_ua
    OPCUA_AVAILABLE = True
    OPCUA_LIB = "opcua"
except ImportError:
    try:
        from asyncua.sync import Client as OpcClient
        from asyncua import ua as opcua_ua
        OPCUA_AVAILABLE = True
        OPCUA_LIB = "asyncua"
    except ImportError:
...
def simulate_plc_realtime(
    ...
    for raw_i in range(n_raw):
        ...
        if cur > 0:
            ...
            if a_step > 180.0:
                a_step = 360.0 - a_step
            total_travel += a_step
        # *** v4.7 *** aggiorna angle_exceeded (come bAngleExceeded SCL)
        if not angle_exceeded and total_travel > window_deg:
            angle_exceeded = True

        # *** v4.6 *** C-bis: rilevamento superficie piatta iniziale
        if flat_wait_enable and not flat_found and cur > 0:
            if abs(buf_s[cur] - buf_s[cur - 1]) <= flat_wait_toll:
                flat_consec += 1
                if flat_consec >= flat_wait_samples:
                    flat_found = True
        ...
        if use_db_thresholds and db_thresh_hi is not None and cur < len(db_thresh_hi):
            ...
            if cur > 0 and use_fast_numpy:
                ...
                if adapt_baseline_enable and adapt_bl_set:
                    mask = mask & (buf_s[:cur] >= adapt_bl_min) & (buf_s[:cur] <= adapt_bl_max)
                win_n = int(mask.sum())
                if win_n >= 3:
                    vs = buf_s[:cur][mask]
                    if use_float32:
                        ...
                        if adapt_baseline_enable:
                            adapt_bl_min = m - adapt_baseline_offset
                            adapt_bl_max = m + adapt_baseline_offset
                            adapt_bl_set = angle_exceeded
                    else:
        ...
        if not in_cluster:
            ...
            if check_positive and las >= th_hi:
                in_cluster = True
            ...
            elif check_negative and las <= th_hi_neg:
                in_cluster = True
    ...
    return {
        ...
        "angle_exceeded": angle_exceeded, # *** v4.7 ***
...
class WeldViewerApp(tk.Tk):
    ...
    def _on_close(self):
        ...
        if hasattr(self, '_stat_executor') and self._stat_executor:
            try:
                self._stat_executor.shutdown(wait=False, cancel_futures=True)
            except TypeError:
                self._stat_executor.shutdown(wait=False)
    ...
    def _build_ui(self):
        ...
        left = ttk.Frame(main, width=295);  main.add(left, weight=0)
    ...
    def _build_right(self, p):
        ...
        #  TAB 4: 🌐 OPC UA
        # ══════════════════════════════════════════════════════
        opcua_frame = ttk.Frame(nb)
        nb.add(opcua_frame, text="  🌐  OPC UA  ")
        self._build_opcua_tab(opcua_frame)
        ...
        self._sim_tab_idx      = 1
        self._plc_tab_idx      = 2
        self._opcua_tab_idx    = 3
        self._stat_tab_idx     = 4
        self._sqlimport_tab_idx= 5
        self._sqlquery_tab_idx = 6
    ...
    def _build_sim_tab(self, parent):
        ...
        lf_outer = ttk.Frame(pane, width=290);  pane.add(lf_outer, weight=0)
        lf_outer.pack_propagate(False)          # *** impedisce resize da contenuto ***
        
        # Canvas scrollabile
        sim_canvas = tk.Canvas(lf_outer, bg=DARK_BG, highlightthickness=0, width=270)
    ...
    def _preload_sim_params(self):
        ...
        for bool_k, sv_var in [('I_AdaptBaselineEnable', '_sv46_adapt_en'),
            ...
            if val is not None and hasattr(self, sv_var):
                getattr(self, sv_var).set(bool(val))
    ...
    def _draw_sim_signal(self):
        ...
        # *** v4.3.98 *** linea verticale dove bFlatSurfaceFound diventa TRUE
        if self._sv46_flat_en.get() and len(fa) > 0 and len(fs) > 0:
            ff  = r.get('flat_found', False)
            ffa = r.get('flat_found_at_angle', None)
            if ff and ffa is not None:
                self.ax_sa1.axvline(ffa, color='#3fb950', linewidth=1.2,
                                    linestyle='--', alpha=0.85, zorder=5,
                                    label=f'✓ Sup.piatta {ffa:.1f}°')
                self.ax_sa2.axvline(ffa, color='#3fb950', linewidth=1.0,
                                    linestyle='--', alpha=0.6, zorder=5)
            elif not ff:
                _ymn = float(np.nanmin(fs)); _ymx = float(np.nanmax(fs))
                self.ax_sa1.text(float(fa[0]), _ymn + (_ymx-_ymn)*0.05,
                                 '✗ Sup.piatta non trovata',
                                 color='#ff6b6b', fontsize=8, va='bottom')
    ...
    def _build_plc_dbreader(self, parent):
        ...
        pane = ttk.PanedWindow(parent, orient="horizontal")
        pane.pack(fill="both", expand=True)
        left = ttk.Frame(pane, width=310);  pane.add(left, weight=0)
    ...
    def _build_plc_autoexport(self, parent):
        ...
        pane = ttk.PanedWindow(parent, orient="horizontal")
        pane.pack(fill="both", expand=True)
        left_outer = ttk.Frame(pane, width=340); pane.add(left_outer, weight=0)
    ...
    def _build_plc_realtime(self, parent):
        ...
        pane = ttk.PanedWindow(parent, orient="horizontal")
        pane.pack(fill="both", expand=True)
        left  = ttk.Frame(pane, width=290);  pane.add(left,  weight=0)
        right = ttk.Frame(pane);             pane.add(right, weight=1)
    ...
    def _stat_load_from_sqlite(self):
        ...
        dt_from = self._pv_stat_sql_from.get().strip()
        dt_to   = self._pv_stat_sql_to.get().strip()
        if dt_from: conditions.append(f"timestamp >= '{dt_from}'")
        if dt_to:   conditions.append(f"timestamp <= '{dt_to}'")
    ...
    def _build_sqlite_import_tab(self, parent):
        ...
        left  = ttk.Frame(main, width=310); main.add(left,  weight=0); left.pack_propagate(False)
        right = ttk.Frame(main);            main.add(right, weight=1)
        # Sorgente
    ...
    def _build_opcua_tab(self, parent):
        """Costruisce il tab OPC UA con flag abilitazione, config, browse e lettura."""
        self._opcua_reader = None
        self._opcua_weldfind_dbs = []
        self._opcua_xml_dbs = None  # Dati da file XML OPC UA (se caricato)

        # ── Layout: sinistra (config) + destra (log + lista) ──
        pane = ttk.PanedWindow(parent, orient="horizontal")
        pane.pack(fill="both", expand=True)

        left = ttk.Frame(pane, width=330);  pane.add(left, weight=0)
        right = ttk.Frame(pane);  pane.add(right, weight=1)

        # ═════════════ FLAG ABILITAZIONE ═══════════════════════
        flag_frm = ttk.LabelFrame(left, text="  Abilitazione  ", padding=8)
        flag_frm.pack(fill="x", padx=6, pady=(6, 2))

        self._pv_opcua_enabled = tk.BooleanVar(value=False)
        chk = tk.Checkbutton(flag_frm, text="  Abilita OPC UA",
                             variable=self._pv_opcua_enabled,
                             bg=DARK_BG, fg=OPCUA_CLR, selectcolor="#1f6feb",
                             activebackground=DARK_BG, activeforeground=OPCUA_CLR,
                             font=("Consolas", 10, "bold"),
                             command=self._opcua_toggle_enable)
        chk.pack(anchor="w")

        self._opcua_enable_lbl = tk.Label(flag_frm, text="",
            bg=DARK_BG, fg=MUTED_CLR, font=("Consolas", 9), anchor="w")
        self._opcua_enable_lbl.pack(fill="x", pady=(2, 0))

        # ═════════════ COMUNICAZIONE ═══════════════════════════
        conn_frm = ttk.LabelFrame(left, text="  Comunicazione OPC UA  ", padding=8)
        conn_frm.pack(fill="x", padx=6, pady=4)

        # IP/Porta
        row1 = ttk.Frame(conn_frm);  row1.pack(fill="x", pady=2)
        ttk.Label(row1, text="IP:", width=6).pack(side="left")
        self._pv_opcua_ip = tk.StringVar(value="172.28.2.1")
        ttk.Entry(row1, textvariable=self._pv_opcua_ip, width=16).pack(side="left", padx=2)
        ttk.Label(row1, text="Porta:").pack(side="left", padx=(8, 0))
        self._pv_opcua_port = tk.StringVar(value="4840")
        ttk.Entry(row1, textvariable=self._pv_opcua_port, width=6).pack(side="left", padx=2)

        # User/Password
        row2 = ttk.Frame(conn_frm);  row2.pack(fill="x", pady=2)
        ttk.Label(row2, text="User:", width=6).pack(side="left")
        self._pv_opcua_user = tk.StringVar()
        ttk.Entry(row2, textvariable=self._pv_opcua_user, width=12).pack(side="left", padx=2)
        ttk.Label(row2, text="Pass:").pack(side="left", padx=(8, 0))
        self._pv_opcua_pass = tk.StringVar()
        ttk.Entry(row2, textvariable=self._pv_opcua_pass, width=12, show="•").pack(side="left", padx=2)

        # Timeout
        row3 = ttk.Frame(conn_frm);  row3.pack(fill="x", pady=2)
        ttk.Label(row3, text="Timeout:", width=8).pack(side="left")
        self._pv_opcua_timeout = tk.StringVar(value="10")
        ttk.Entry(row3, textvariable=self._pv_opcua_timeout, width=5).pack(side="left", padx=2)
        ttk.Label(row3, text="sec", style="Muted.TLabel").pack(side="left", padx=2)

        # Endpoint preview
        self._pv_opcua_endpoint_lbl = tk.Label(conn_frm, text="",
            bg=DARK_BG, fg=MUTED_CLR, font=("Consolas", 8), anchor="w")
        self._pv_opcua_endpoint_lbl.pack(fill="x", pady=(4, 0))
        # Aggiorna preview dinamicamente
        for var in (self._pv_opcua_ip, self._pv_opcua_port):
            var.trace_add("write", self._opcua_update_endpoint_preview)
        self._opcua_update_endpoint_preview()

        # ═════════════ AZIONI ══════════════════════════════════
        act_frm = ttk.LabelFrame(left, text="  Azioni  ", padding=8)
        act_frm.pack(fill="x", padx=6, pady=4)

        # XML load — sempre abilitato (non richiede connessione)
        self._btn_opcua_xml = ttk.Button(act_frm, text="📄  Carica XML OPC UA",
                                          command=self._opcua_load_xml, style="OpcUa.TButton")
        self._btn_opcua_xml.pack(fill="x", pady=2, padx=4)

        ttk.Separator(act_frm, orient="horizontal").pack(fill="x", padx=4, pady=4)

        # Pulsanti lettura — controllati dal flag (ogni bottone è autonomo: connette, legge, disconnette)
        read_btn_data = [
            ("📥  Connetti + Leggi DB",           self._opcua_read_selected, "Accent.TButton"),
            ("💾  Connetti + Leggi + Salva .db",  self._opcua_save_selected, "TButton"),
            ("📊  Connetti + Leggi + Viewer",     self._opcua_load_viewer,   "Accent.TButton"),
        ]
        self._opcua_read_btns = []  # solo questi sono controllati dal flag
        for text, cmd, style in read_btn_data:
            b = ttk.Button(act_frm, text=text, command=cmd, style=style)
            b.pack(fill="x", pady=2, padx=4)
            self._opcua_read_btns.append(b)

        # ═════════════ RISULTATO ═══════════════════════════════
        res_frm = ttk.LabelFrame(left, text="  Risultato  ", padding=6)
        res_frm.pack(fill="x", padx=6, pady=4)

        self._opcua_result_vars = {}
        for label in ("Stato:", "Server:", "Namespace:", "XML:", "DB Trovati:", "Selezionato:",
                       "Campioni:", "Polarità:"):
            row = ttk.Frame(res_frm);  row.pack(fill="x", pady=1)
            ttk.Label(row, text=label, width=12, anchor="w").pack(side="left", padx=2)
            sv = tk.StringVar(value="—")
            self._opcua_result_vars[label] = sv
            lbl_style = "Result.TLabel"
            ttk.Label(row, textvariable=sv, style=lbl_style).pack(side="left")

        self._pv_opcua_status = tk.StringVar(value="Disabilitato")
        ttk.Label(left, textvariable=self._pv_opcua_status, style="Muted.TLabel",
                  anchor="w").pack(fill="x", padx=10, pady=(4, 2))

        # ═════════════ DESTRA: LISTA DB + LOG ══════════════════
        # Treeview DB trovati
        tree_frm = ttk.LabelFrame(right, text="  DB WeldFind trovati  ", padding=4)
        tree_frm.pack(fill="both", expand=True, padx=6, pady=(6, 2))

        columns = ("name", "samples", "polarity", "state", "match", "vars")
        self._opcua_tree = ttk.Treeview(tree_frm, columns=columns, show="headings", height=8)
        self._opcua_tree.heading("name",     text="Nome DB")
        self._opcua_tree.heading("samples",  text="Campioni")
        self._opcua_tree.heading("polarity", text="Polarità")
        self._opcua_tree.heading("state",    text="Stato")
        self._opcua_tree.heading("match",    text="Match")
        self._opcua_tree.heading("vars",     text="Variabili")
        self._opcua_tree.column("name",     width=200)
        self._opcua_tree.column("samples",  width=70,  anchor="center")
        self._opcua_tree.column("polarity", width=80,  anchor="center")
        self._opcua_tree.column("state",    width=65,  anchor="center")
        self._opcua_tree.column("match",    width=55,  anchor="center")
        self._opcua_tree.column("vars",     width=65,  anchor="center")

        tsb = ttk.Scrollbar(tree_frm, orient="vertical", command=self._opcua_tree.yview)
        self._opcua_tree.configure(yscrollcommand=tsb.set)
        self._opcua_tree.pack(side="left", fill="both", expand=True)
        tsb.pack(side="right", fill="y")

        self._opcua_tree.bind("<<TreeviewSelect>>", self._opcua_on_tree_select)
        self._opcua_tree.bind("<Double-1>", lambda e: self._opcua_load_viewer())

        # Log
        log_frm = ttk.LabelFrame(right, text="  Log OPC UA  ", padding=4)
        log_frm.pack(fill="both", expand=True, padx=6, pady=(2, 6))

        lsb = ttk.Scrollbar(log_frm);  lsb.pack(side="right", fill="y")
        self._opcua_log = tk.Text(log_frm, bg=DARK_BG, fg=TEXT_CLR, font=("Consolas", 9),
                                   wrap="word", height=10, yscrollcommand=lsb.set,
                                   insertbackground=TEXT_CLR, selectbackground=ACCENT)
        self._opcua_log.pack(fill="both", expand=True)
        lsb.config(command=self._opcua_log.yview)

        self._opcua_log.tag_config("ok",   foreground=OK_CLR)
        self._opcua_log.tag_config("err",  foreground=WELD_CLR)
        self._opcua_log.tag_config("info", foreground=OPCUA_CLR)
        self._opcua_log.tag_config("warn", foreground=WARN_CLR)

        # Messaggio iniziale
        self._opcua_log_msg("═══ OPC UA WeldFind Reader — v4.3.6 ═══\n\n", "info")
        if OPCUA_AVAILABLE:
            self._opcua_log_msg(f"✓ Libreria OPC UA disponibile ({OPCUA_LIB})\n\n", "ok")
        else:
            self._opcua_log_msg("⚠ Libreria OPC UA NON installata!\n", "err")
            self._opcua_log_msg("  Abilita il flag → verrà proposta l'installazione automatica.\n", "warn")
            self._opcua_log_msg("  Oppure manualmente: pip install opcua\n\n", "warn")
        self._opcua_log_msg("PREREQUISITI PLC:\n", "info")
        self._opcua_log_msg("  1. Server OPC UA abilitato nella CPU\n")
        self._opcua_log_msg("     (Proprietà CPU → OPC UA → Attiva server)\n")
        self._opcua_log_msg("  2. Per ogni DB WeldFind:\n")
        self._opcua_log_msg("     DB_Accessible_From_OPC_UA := 'TRUE'\n")
        self._opcua_log_msg("     (Proprietà DB → Attributi → Accessibile da OPC UA)\n")
        self._opcua_log_msg("  3. S7_Optimized_Access := 'FALSE' (già ok)\n\n")
        self._opcua_log_msg("FLUSSO:\n", "info")
        self._opcua_log_msg("  1) Carica XML OPC UA → trova DB istantaneamente\n")
        self._opcua_log_msg("  2) Abilita flag lettura + configura IP/porta\n")
        self._opcua_log_msg("  3) Seleziona DB → Connetti+Leggi → Viewer\n")
        self._opcua_log_msg("     (ogni pulsante è autonomo: connette, legge, disconnette)\n\n")

        # Stato iniziale: tutto disabilitato
        self._opcua_toggle_enable()

    # ── OPC UA: utilità ──────────────────────────────────────
    def _opcua_log_msg(self, msg, tag=None):
        self._opcua_log.insert("end", msg, tag if tag else ())
        self._opcua_log.see("end");  self.update_idletasks()

    def _opcua_log_clear(self):
        self._opcua_log.delete("1.0", "end")

    def _opcua_set_result(self, key, val):
        if key in self._opcua_result_vars:
            self._opcua_result_vars[key].set(str(val))

    def _opcua_update_endpoint_preview(self, *_args):
        ip = self._pv_opcua_ip.get().strip()
        port = self._pv_opcua_port.get().strip()
        self._pv_opcua_endpoint_lbl.config(text=f"  Endpoint: opc.tcp://{ip}:{port}")

    def _opcua_get_endpoint(self):
        ip = self._pv_opcua_ip.get().strip()
        port = self._pv_opcua_port.get().strip()
        return f"opc.tcp://{ip}:{port}"

    def _opcua_toggle_enable(self):
        """Abilita/disabilita solo i pulsanti di lettura dati OPC UA.
        Il caricamento XML è sempre disponibile.
        All'abilitazione, verifica che la libreria opcua sia installata."""
        enabled = self._pv_opcua_enabled.get()
        state = "normal" if enabled else "disabled"
        for btn in self._opcua_read_btns:
            btn.configure(state=state)
        if enabled:
            self._opcua_enable_lbl.config(text="✓ Lettura OPC UA abilitata", fg=OK_CLR)
            self._opcua_log_msg("✓ Lettura OPC UA abilitata\n", "ok")
            # ── Check automatico librerie ──
            self._opcua_auto_check_libs()
        else:
            self._opcua_enable_lbl.config(text="Lettura OPC UA disabilitata", fg=MUTED_CLR)
            self._pv_opcua_status.set("Lettura disabilitata")

    def _opcua_auto_check_libs(self):
        """Check automatico librerie OPC UA all'abilitazione del flag.
        Se mancanti, propone installazione e ricarica il modulo."""
        global OPCUA_AVAILABLE, OPCUA_LIB, OpcClient, opcua_ua

        # Controlla se opcua è già disponibile
        if OPCUA_AVAILABLE:
            self._opcua_log_msg(f"✓ Libreria OPC UA disponibile ({OPCUA_LIB})\n", "ok")
            self._pv_opcua_status.set("Pronto — configura e connetti")
            return

        # Tenta import fresco (potrebbe essere stato installato dopo l'avvio)
        try:
            from opcua import Client as _C, ua as _U
            OpcClient = _C;  opcua_ua = _U
            OPCUA_AVAILABLE = True;  OPCUA_LIB = "opcua"
            self._opcua_log_msg("✓ Libreria opcua trovata!\n", "ok")
            self._pv_opcua_status.set("Pronto — configura e connetti")
            return
        except ImportError:
            pass
        try:
            from asyncua.sync import Client as _C
            from asyncua import ua as _U
            OpcClient = _C;  opcua_ua = _U
            OPCUA_AVAILABLE = True;  OPCUA_LIB = "asyncua"
            self._opcua_log_msg("✓ Libreria asyncua trovata!\n", "ok")
            self._pv_opcua_status.set("Pronto — configura e connetti")
            return
        except ImportError:
            pass

        # Non trovata: proponi installazione
        self._opcua_log_msg("⚠ Libreria OPC UA non installata!\n", "err")
        self._pv_opcua_status.set("Libreria mancante!")

        msg = ("La libreria OPC UA (python-opcua) non è installata.\n\n"
               "È necessaria per la comunicazione OPC UA col PLC.\n\n"
               "Vuoi installarla ora?\n"
               "(equivale a: pip install opcua)")
        if not messagebox.askyesno("Installazione OPC UA", msg, parent=self):
            self._opcua_log_msg("Installazione annullata. OPC UA non disponibile.\n", "warn")
            self._pv_opcua_status.set("Libreria mancante — installa opcua")
            return

        # ── Esegui pip install ──
        import subprocess, sys
        self._opcua_log_msg("Installazione opcua in corso...\n", "info")
        self._pv_opcua_status.set("Installazione opcua...")
        self.update_idletasks()

        try:
            result = subprocess.run(
                [sys.executable, "-m", "pip", "install", "opcua"],
                capture_output=True, text=True, timeout=120
            )
            if result.returncode == 0:
                self._opcua_log_msg("✓ opcua installata con successo!\n", "ok")
                # Ricarica il modulo
                try:
                    from opcua import Client as _C, ua as _U
                    OpcClient = _C;  opcua_ua = _U
                    OPCUA_AVAILABLE = True;  OPCUA_LIB = "opcua"
                    self._opcua_log_msg("✓ Modulo opcua caricato e pronto!\n\n", "ok")
                    self._pv_opcua_status.set("Pronto — configura e connetti")
                except ImportError as ie:
                    self._opcua_log_msg(f"⚠ Installato ma import fallito: {ie}\n", "warn")
                    self._opcua_log_msg("  Riavvia l'applicazione.\n\n", "warn")
                    self._pv_opcua_status.set("Riavvia per completare")
            else:
                self._opcua_log_msg("✗ Errore installazione:\n", "err")
                if result.stderr:
                    for line in result.stderr.strip().split('\n')[-5:]:
                        self._opcua_log_msg(f"  {line}\n", "err")
                self._opcua_log_msg(f"\nProva manualmente:\n  {sys.executable} -m pip install opcua\n\n", "warn")
                self._pv_opcua_status.set("Installazione fallita")
        except subprocess.TimeoutExpired:
            self._opcua_log_msg("✗ Timeout installazione (>120s)\n", "err")
            self._pv_opcua_status.set("Timeout installazione")
        except Exception as e:
            self._opcua_log_msg(f"✗ Errore: {e}\n", "err")
            self._pv_opcua_status.set("Errore installazione")

    def _opcua_on_tree_select(self, event=None):
        """Aggiorna info risultato quando si seleziona un DB nella lista."""
        sel = self._opcua_tree.selection()
        if not sel:
            self._opcua_set_result("Selezionato:", "—")
            return
        idx = self._opcua_tree.index(sel[0])
        if idx < len(self._opcua_weldfind_dbs):
            db = self._opcua_weldfind_dbs[idx]
            self._opcua_set_result("Selezionato:", db["name"])
            self._opcua_set_result("Campioni:", str(db["samples_acquired"]))
            pol_names = {0: "Positivo", 1: "Negativo", 2: "Entrambi"}
            self._opcua_set_result("Polarità:", pol_names.get(db["peak_polarity"], "?"))

    # ── OPC UA: CARICA XML ─────────────────────────────────────
    def _opcua_load_xml(self):
        """Carica file XML OPC UA e trova istantaneamente i DB WeldFind.
        Sempre disponibile, non richiede connessione né flag abilitazione."""
        path = filedialog.askopenfilename(
            parent=self, title="Seleziona file XML OPC UA",
            filetypes=[("XML files", "*.xml"), ("Tutti i file", "*.*")])
        if not path:
            return

        file_size = os.path.getsize(path)
        self._opcua_log_msg(f"\n{'─'*40}\n", "info")
        self._opcua_log_msg(f"Caricamento XML: {os.path.basename(path)}\n", "info")
        self._opcua_log_msg(f"  Dimensione: {file_size/1024/1024:.1f} MB\n")
        self._pv_opcua_status.set("Analisi XML in corso...")
        self.update_idletasks()

        t0 = time.time()
        try:
            def progress(pct, msg):
                self._pv_opcua_status.set(f"XML... {pct}% — {msg}")
                self.update_idletasks()

            xml_dbs = opcua_parse_xml_weldfind(path, callback=progress)
        except Exception as e:
            self._opcua_log_msg(f"✗ Errore parsing XML: {e}\n", "err")
            self._pv_opcua_status.set("Errore XML!")
            return

        elapsed = time.time() - t0
        self._opcua_log_msg(f"✓ XML analizzato in {elapsed:.1f}s\n", "ok")

        # Salva per uso successivo
        self._opcua_xml_dbs = xml_dbs

        # Popola treeview
        self._opcua_tree.delete(*self._opcua_tree.get_children())
        self._opcua_weldfind_dbs = []

        for db in xml_dbs:
            # Converto in formato compatibile col browse live
            db_info = {
                "name": db["name"],
                "node_id": db["node_id"],
                "samples_acquired": 0,  # Non noto dall'XML (serve lettura)
                "peak_polarity": 0,
                "state": 0,
                "match_score": db["var_count"],
                "source": "xml",
                "xml_data": db,  # Mantieni dati XML per lettura veloce
            }
            self._opcua_weldfind_dbs.append(db_info)

            var_names = sorted(db["variables"].keys())
            # Conta variabili note
            known_scalars = set(_OPCUA_ALL_SCALARS) & set(var_names)
            known_arrays = set(_PLC_ARRAY_ORDER) & set(var_names)

            self._opcua_tree.insert("", "end", values=(
                db["name"],
                "—",  # campioni non noti dall'XML
                "—",
                "—",
                f"{len(known_scalars)}+{len(known_arrays)}a",
                db["var_count"],
            ))

            self._opcua_log_msg(
                f"  ✓ {db['name']}: {db['var_count']} variabili "
                f"({len(known_scalars)} scalari, {len(known_arrays)} array)\n", "ok")

        n = len(xml_dbs)
        self._opcua_set_result("Stato:", f"✓ XML: {n} DB")
        self._opcua_set_result("XML:", f"✓ {os.path.basename(path)}")
        self._opcua_set_result("DB Trovati:", str(n))
        self._pv_opcua_status.set(f"XML: {n} DB WeldFind trovati in {elapsed:.1f}s")

        if n == 0:
            self._opcua_log_msg(
                "\n⚠ Nessun DB WeldFind trovato nell'XML.\n"
                "  L'XML contiene DB istanza di Fb954_WeldFindV43?\n"
                "  Verificare che DB_Accessible_From_OPC_UA = TRUE.\n", "warn")
        else:
            self._opcua_log_msg(
                f"\n✓ {n} DB WeldFind trovati in {elapsed:.1f}s!\n"
                f"  Seleziona un DB, abilita il flag e premi 'Connetti + Leggi'.\n"
                f"  I NodeId dall'XML consentono accesso diretto (molto veloce).\n\n", "ok")

    # ── OPC UA: TEST CONNESSIONE ─────────────────────────────
    # ── OPC UA: LETTURA DB ───────────────────────────────────
    def _opcua_get_selected_db(self):
        """Ritorna il dict del DB selezionato nel treeview, o None."""
        sel = self._opcua_tree.selection()
        if not sel:
            self._opcua_log_msg("⚠ Seleziona un DB dalla lista!\n", "warn")
            return None
        idx = self._opcua_tree.index(sel[0])
        if idx < len(self._opcua_weldfind_dbs):
            return self._opcua_weldfind_dbs[idx]
        return None

    def _opcua_read_db(self, db_name, xml_data=None):
        """Connette, legge il DB, disconnette. Ritorna dict dati o None.
        
        Se xml_data è fornito (da XML OPC UA), usa i NodeId pre-estratti
        per accesso diretto molto più veloce.
        """
        if not OPCUA_AVAILABLE:
            self._opcua_log_msg("✗ Libreria OPC UA non disponibile.\n", "err")
            return None

        endpoint = self._opcua_get_endpoint()
        user = self._pv_opcua_user.get().strip() or None
        pw = self._pv_opcua_pass.get().strip() or None
        timeout = float(self._pv_opcua_timeout.get() or 10)

        reader = OpcUaWeldFindReader(endpoint, timeout=timeout,
                                      username=user, password=pw)
        try:
            reader.connect()
            self._opcua_log_msg(f"✓ Connesso a {endpoint}\n", "ok")

            def progress(pct, msg):
                self._pv_opcua_status.set(f"Lettura... {pct}% — {msg}")
                self.update_idletasks()

            if xml_data:
                # Lettura veloce con NodeId dall'XML
                self._opcua_log_msg("  Modalità: XML-guided (accesso diretto per NodeId)\n", "info")
                data = reader.read_weldfind_db_xml(xml_data, callback=progress)
            else:
                # Lettura standard con browse
                self._opcua_log_msg("  Modalità: browse live\n", "info")
                data = reader.read_weldfind_db(db_name, callback=progress)

            reader.disconnect()

            n = int(data["scalars"].get("iSamplesAcquired", 0))
            self._opcua_log_msg(
                f"✓ DB '{db_name}': {n} campioni, "
                f"{len(data['arrays'])} array letti\n", "ok")
            return data

        except Exception as e:
            self._opcua_log_msg(f"✗ Errore lettura: {e}\n", "err")
            try:
                reader.disconnect()
            except Exception:
                pass
    ...
    def _opcua_read_selected(self):
        """Leggi il DB selezionato (non bloccante)."""
        if not self._pv_opcua_enabled.get(): return
        db = self._opcua_get_selected_db()
        if not db: return
        self._opcua_log_msg(f"\n{'-'*40}\n", "info")
        self._opcua_log_msg(f"Lettura DB '{db['name']}'...\n", "info")

        def _read():
            return self._opcua_read_db(db["name"], xml_data=db.get("xml_data"))

        def _done(data):
            if not data:
                self._pv_opcua_status.set("Errore lettura!"); return
            sc = data["scalars"]
            n  = int(sc.get("iSamplesAcquired", 0))
            pol_names = {0:"Positivo",1:"Negativo",2:"Entrambi"}
            self._opcua_set_result("Campioni:", str(n))
            self._opcua_set_result("Polarità:", pol_names.get(int(sc.get("I_PeakPolarity",0)),"?"))
            self._opcua_set_result("Stato:", f"✓ Letto {n} camp.")
            self._pv_opcua_status.set(f"DB '{db['name']}' - {n} campioni")
            self._opcua_log_msg(f"  Peak: {sc.get('rPeakValue',0):.4f}\n")
            self._opcua_log_msg(f"  Dev:  {sc.get('rPeakDeviation',0):.4f}\n")
            self._opcua_last_data   = data
            self._opcua_last_dbname = db["name"]
            self._opcua_log_msg("✓ Dati pronti per il viewer.\n\n", "ok")

        self._run_in_thread(_read, on_done=_done,
                            status_var=self._pv_opcua_status,
                            status_msg=f"Lettura {db['name']}...")

    def _opcua_save_selected(self):
        """Leggi il DB selezionato e salva come file .db."""
        if not self._pv_opcua_enabled.get():
            return
        db = self._opcua_get_selected_db()
        if not db:
            return
        self._opcua_log_msg(f"\n{'─'*40}\n", "info")
        self._opcua_log_msg(f"Lettura DB '{db['name']}' per salvataggio...\n", "info")
        self._pv_opcua_status.set(f"Lettura {db['name']}...")

        data = self._opcua_read_db(db["name"], xml_data=db.get("xml_data"))
        if not data:
            self._pv_opcua_status.set("Errore lettura!")
            return

        ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        path = filedialog.asksaveasfilename(
            parent=self, title="Salva file .db",
            defaultextension=".db",
            filetypes=[("DB files", "*.db"), ("Tutti i file", "*.*")],
            initialfile=f"{db['name']}_{ts}.db"
        )
        if path:
            with open(path, 'w', encoding='utf-8') as f:
                f.write(data["raw_text"])
            self._opcua_log_msg(f"✓ Salvato: {path}\n", "ok")
            self._pv_opcua_status.set(f"Salvato: {os.path.basename(path)}")
        else:
            self._pv_opcua_status.set("Salvataggio annullato")

    def _opcua_load_viewer(self):
        """Leggi il DB selezionato e carica nel viewer (non bloccante)."""
        if not self._pv_opcua_enabled.get(): return
        db = self._opcua_get_selected_db()
        if not db: return
        self._opcua_log_msg(f"\n{'-'*40}\n", "info")
        self._opcua_log_msg(f"Lettura DB '{db['name']}' per viewer...\n", "info")
        ...
        def _done(data):
            if not data:
                self._pv_opcua_status.set("Errore lettura!"); return
            self.db_data = data
            filename = data.get("filename", f"OpcUA_{db['name']}.db")
            self.lbl_file.config(text=f"🌐 {filename}")
            self._update_results_panel(); self._preload_sim_params()
            self._recompute();            self._update_raw_tab()
            n = int(data["scalars"].get("iSamplesAcquired", 0))
            self._opcua_log_msg(f"\n✓ Caricato: {filename} ({n} camp.)\n", "ok")
            self._pv_opcua_status.set(f"Viewer: {filename}")
            self.nb.select(0); self._sub_nb.select(0)
    ...
    def _build_stat_tab(self, parent):
        ...
        ttk.Entry(sql_r4, textvariable=self._pv_stat_sql_db, width=10).pack(side="left", padx=2)
        ...
        tbl_lf = ttk.LabelFrame(right, text="  Top risultati (per % trovati + SNR)  ", padding=4)
        ...
        self._stat_tree = ttk.Treeview(tbl_lf, show="headings", height=5,
    ...
    def _stat_update_results(self):
        ...
        best_pct = -1.0; best_ci = 0; best_snr_med = 0.0; best_snr_min = 0.0
        ...
        for ci in range(n_combos):
            ...
            if (pct > best_pct) or (pct == best_pct and snr_meds[ci] > best_snr_med):
                best_pct = pct; best_ci = ci
    ...
    def _stat_stop(self, silent=False):
        ...
        if self._stat_executor:
            ...
            except TypeError: self._stat_executor.shutdown(wait=False)
            self._stat_executor = None
