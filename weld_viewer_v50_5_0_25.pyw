...
def simulate_plc_realtime(
    ...
    for raw_i in range(n_raw):
        ...
        # *** v5.0 *** SCL blocco A: angle_exceeded controlla total_travel PRIMA
        # di aggiornarlo col passo corrente (corrisponde a bAngleExceeded in SCL
        # che e' calcolato nel blocco A, prima che blocco C aggiorni rTotalTravel)
...
class WeldViewerApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self._btn_util = ttk.Button(top, text="🔧  Utilità",
                   command=self._open_utilities_menu)
        self._btn_util.pack(side="left", padx=2)
        ttk.Button(top, text="\u2699\ufe0f  Impostazioni",
        ...
        # ── Bottoni destra: sorgente dati ──
        ttk.Button(top, text="🗄  Da SQLite", style="Accent.TButton",
                   command=self._open_from_sqlite).pack(side="right", padx=4)
    ...
    def _build_left(self, p):
        ...
        self._out_vars = {}  # rimosso riquadro Stato FB

        self._udt_vars = {}  # rimosso riquadro UDT IO_RicercaSaldatura
    ...
    def _build_right(self, p):
        ...
        # Pannello risultati (sinistra) + sub-notebook (destra) dentro il tab Analisi
        analisi_pane = ttk.PanedWindow(analisi_frame, orient="horizontal")
        analisi_pane.pack(fill="both", expand=True)
        left_a = ttk.Frame(analisi_pane, width=210)
        analisi_pane.add(left_a, weight=0)
        left_a.pack_propagate(False)
        self._left_frame = left_a
        ...
        self.fig2 = Figure(facecolor=DARK_BG, figsize=(10, 4.5))
        # ax2: profilo normalizzato (70%) | ax2b: profilo reale cilindro (30%)
        self.ax2  = self.fig2.add_axes([0.04, 0.05, 0.55, 0.85], projection="polar")
        self.ax2b = self.fig2.add_axes([0.66, 0.05, 0.30, 0.85], projection="polar")
        self._style_polar(self.ax2);  self._style_polar(self.ax2b)
        self.canvas2 = FigureCanvasTkAgg(self.fig2, t)
        self.canvas2.get_tk_widget().pack(fill="both", expand=True)
        NavigationToolbar2Tk(self.canvas2, t).pack(fill="x")
        ...
        # SQLite Import e Query accessibili da bottone Utilità (non tab fisse)
        self._build_sqlite_import_tab(sql_imp_frame)
        self._sql_qry_frame_built = False
        self._sql_imp_frame  = sql_imp_frame   # tenuto in memoria ma non aggiunto al nb
        self._sql_qry_frame  = ttk.Frame(nb)   # idem
        self._build_sqlite_query_tab(self._sql_qry_frame)
    ...
    def _on_tab_changed(self, event=None):
        """Cambio tab principale — barra sempre in posizione fissa."""
        pass
    ...
    def _build_sim_tab(self, parent):
        ...
        # Grafici simulazione direttamente in rf (Sorgente rimossa v5.0)
        ...
        self.fig_sc = Figure(facecolor=DARK_BG, figsize=(10, 4.5))
        self.ax_sc  = self.fig_sc.add_axes([0.04, 0.05, 0.55, 0.85], projection="polar")
        self.ax_scb = self.fig_sc.add_axes([0.66, 0.05, 0.30, 0.85], projection="polar")
        self._style_polar(self.ax_sc);  self._style_polar(self.ax_scb)
        self.canvas_sc = FigureCanvasTkAgg(self.fig_sc, sc3)
        self.canvas_sc.get_tk_widget().pack(fill="both", expand=True)
        NavigationToolbar2Tk(self.canvas_sc, sc3).pack(fill="x")
    ...
    # ── OPEN FILE ─────────────────────────────────────────────
    _MANUAL_B64 = ""
    ...
    def _open_utilities_menu(self):
        ...
        dlg.resizable(False, False)
        dlg.grab_set()
        ttk.Label(dlg, text="Apri strumento:", style="Muted.TLabel").pack(pady=(18, 8))
        bf = ttk.Frame(dlg); bf.pack()
        def _open_imp():
            dlg.destroy()
            win = tk.Toplevel(self)
            win.title("SQLite Import")
            win.configure(bg=DARK_BG)
            win.geometry("900x600")
            frm = ttk.Frame(win); frm.pack(fill="both", expand=True)
            self._build_sqlite_import_tab(frm)
        def _open_qry():
            dlg.destroy()
            win = tk.Toplevel(self)
            win.title("SQLite Query")
            win.configure(bg=DARK_BG)
            win.geometry("900x600")
            frm = ttk.Frame(win); frm.pack(fill="both", expand=True)
            self._build_sqlite_query_tab(frm)
        ttk.Button(bf, text="SQLite Import", style="Accent.TButton", width=20,
                   command=_open_imp).pack(side="left", padx=8)
        ttk.Button(bf, text="SQLite Query", style="Sim.TButton", width=20,
                   command=_open_qry).pack(side="left", padx=8)

    def _open_from_sqlite(self):
        import sqlite3 as _sq3
        default_path = (getattr(self, '_pv_global_sql_path', None)
                        and self._pv_global_sql_path.get().strip()
        ...
        ttk.Label(bar, text="SQLite:", style="Muted.TLabel").pack(side="left")
        _saved = getattr(self, '_dlg_sqlite_state', {})
        pv = self._pv_global_sql_path  # usa path dalle impostazioni
        tk.Label(bar, textvariable=pv, bg=DARK_BG, fg=MUTED_CLR,
                 font=("Consolas",8), anchor="w").pack(side="left", padx=4, fill="x", expand=True)

        flt = ttk.Frame(dlg); flt.pack(fill="x", padx=8, pady=2)
        ttk.Label(flt, text="DB:", style="Muted.TLabel").pack(side="left")
        db_v = tk.StringVar(value=_saved.get('db', ''))
        ...
        found_v = tk.StringVar(value=_saved.get('found', 'Tutti'))
        ...
        def _save_state():
            """Persiste i filtri del dialog per la prossima apertura."""
            self._dlg_sqlite_state = {
                'path':  pv.get(),
                'db':    db_v.get(),
                'found': found_v.get(),
            }
        ...
        ttk.Button(btn_bar, text="Chiudi", command=lambda:(_save_state(),dlg.destroy())).pack(side="right")
        # *** v5.0 *** se c'era un path salvato, carica subito la lista
        if _saved.get('path') and os.path.isfile(_saved.get('path','')):
            _refresh()
    ...
    def _recompute(self):
        ...
        if hasattr(self, '_param_vars'):
            self._param_vars["window_deg"].set(f"{win}")
            self._param_vars["sigma_factor"].set(f"{sf}")
            self._param_vars["min_abs_dev"].set(f"{mad}")
    ...
    def _run_simulation(self):
        ...
        try:
            ...
            kwargs = dict(
                ...
                detection_start_deg = float(sv['det_start'].get()) if 'det_start' in sv else 0.0,  # *** v4.9 ***
            )
    ...
    def _draw_sim_polar(self):
        ...
        # Vista profilo reale cilindro
        da_sim  = r["detection_angle"] if r["detection_sample"] >= 0 else None
        ws_sim  = r.get("weld_start"); we_sim = r.get("weld_end")
        self._draw_polar_cylinder_profile(self.ax_scb, fa, fs,
                                          det_angle=da_sim, weld_start=ws_sim,
                                          weld_end=we_sim if ws_sim != we_sim else None,
                                          color=SIM_CLR)
    ...
    def _build_multidb_tab(self, parent):
        ...
        ttk.Label(top, text="Sovrapponi profili da SQLite (es. Anello vs Virola)",
                  style="Muted.TLabel").pack(side="left")
        ttk.Button(top, text="🗄  Riga 2° da SQLite", style="Accent.TButton",
                   command=self._open_second_from_sqlite).pack(side="right", padx=4)
        self.lbl_file2 = ttk.Label(top, text="Nessuna 2° riga", style="Muted.TLabel")
    ...
    def _open_second_from_sqlite(self):
        """Apre dialog SQLite per selezionare la riga del 2° profilo nel confronto multi-DB."""
        import sqlite3 as _sq3
        default_path = getattr(self, '_dlg_sqlite_state', {}).get('path',
            self._pv_global_sql_path.get() or
            os.path.join(os.path.expanduser('~'), 'WeldExport', 'weld_archive.sqlite'))
        dlg = tk.Toplevel(self)
        dlg.title("Seleziona riga SQLite - 2° profilo confronto")
        dlg.configure(bg=DARK_BG); dlg.geometry("840x500"); dlg.grab_set()
        bar = ttk.Frame(dlg); bar.pack(fill="x", padx=8, pady=6)
        ttk.Label(bar, text="File SQLite:", style="Muted.TLabel").pack(side="left")
        _saved2 = getattr(self, '_dlg_sqlite2_state', {})
        pv = tk.StringVar(value=_saved2.get('path', default_path))
        ttk.Entry(bar, textvariable=pv, width=50).pack(side="left", padx=4, fill="x", expand=True)
        ttk.Button(bar, text="📁", width=3, command=lambda: pv.set(
            filedialog.askopenfilename(title="Seleziona SQLite",
                filetypes=[("SQLite","*.sqlite *.db3 *.db"),("Tutti","*.*")]) or pv.get())
        ).pack(side="left")
        flt = ttk.Frame(dlg); flt.pack(fill="x", padx=8, pady=2)
        ttk.Label(flt, text="DB:", style="Muted.TLabel").pack(side="left")
        db_v = tk.StringVar(value=_saved2.get('db',''))
        ttk.Entry(flt, textvariable=db_v, width=8).pack(side="left", padx=(2,10))
        ttk.Label(flt, text="Trovata:", style="Muted.TLabel").pack(side="left")
        found_v = tk.StringVar(value=_saved2.get('found','Tutti'))
        ttk.Combobox(flt, textvariable=found_v, width=8, state="readonly",
                     values=["Tutti","Si","No"]).pack(side="left", padx=2)
        n_lbl_v = tk.StringVar(value="")
        ttk.Button(flt, text="🔍 Aggiorna", command=lambda: _refresh()).pack(side="left", padx=8)
        ttk.Label(flt, textvariable=n_lbl_v, style="Muted.TLabel").pack(side="left")
        tree_frm = ttk.Frame(dlg); tree_frm.pack(fill="both", expand=True, padx=8, pady=4)
        cols = ("id","timestamp","db","trovata","n_samp","det_ang","snr","filename")
        tree = ttk.Treeview(tree_frm, columns=cols, show="headings", height=16)
        for col, lbl, w in [("id","ID",50),("timestamp","Timestamp",140),("db","DB",60),
            ("trovata","Trovata",60),("n_samp","Camp.",55),
            ("det_ang","Ang",65),("snr","SNR",60),("filename","File",220)]:
            tree.heading(col, text=lbl); tree.column(col, width=w, anchor="center" if col!="filename" else "w")
        tree.tag_configure("found", background="#0f2d0f", foreground=OK_CLR)
        tree.tag_configure("nofnd", background=ENTRY_BG, foreground=TEXT_CLR)
        sb = ttk.Scrollbar(tree_frm, orient="vertical", command=tree.yview)
        tree.configure(yscrollcommand=sb.set)
        ...
        def _refresh():
            ...
            _rows_cache = []
            path = pv.get().strip()
            if not path or not os.path.isfile(path): n_lbl_v.set("File non trovato"); return
            try:
                con = _sq3.connect(path)
                where = []
                if db_v.get().strip().isdigit(): where.append(f"db_number={db_v.get().strip()}")
                if found_v.get()=="Si": where.append("weld_found=1")
                elif found_v.get()=="No": where.append("weld_found=0")
                w_sql=("WHERE "+" AND ".join(where)) if where else ""
                rows=con.execute(f"SELECT * FROM acquisitions {w_sql} ORDER BY timestamp DESC LIMIT 500").fetchall()
                con.close(); _rows_cache=rows
                for r in rows:
                    tag="found" if r[4] else "nofnd"
                    tree.insert("","end",iid=str(r[0]),tags=(tag,),values=(
                        r[0],str(r[1])[:19] if r[1] else "",r[2],
                        "\u2713" if r[4] else "\u2717",
                        r[10],f"{r[5]:.1f}" if r[5] else "",
                        f"{r[9]:.2f}" if r[9] else "",r[3] or ""))
                n_lbl_v.set(f"{len(rows)} righe")
            except Exception as e: n_lbl_v.set(f"Errore: {e}")
        def _save_state2():
            self._dlg_sqlite2_state={'path':pv.get(),'db':db_v.get(),'found':found_v.get()}
        def _load_selected():
            _save_state2()
            sel=tree.selection()
            if not sel: return
            ...
            try:
                ...
                self.db_data2=data
                fname=data.get('filename',f'SQLite_row{row_id}')
                self.lbl_file2.config(text=fname)
                self._draw_multidb(); dlg.destroy()
            except Exception as e:
                messagebox.showerror("Errore caricamento", str(e), parent=dlg)
        tree.bind("<Double-1>", lambda e: _load_selected())
        btn_bar=ttk.Frame(dlg); btn_bar.pack(fill="x", padx=8, pady=(2,8))
        ttk.Button(btn_bar, text="\u2714 Carica come 2° profilo", style="Accent.TButton",
                   command=_load_selected).pack(side="left")
        ttk.Button(btn_bar, text="Chiudi",
                   command=lambda:(_save_state2(),dlg.destroy())).pack(side="right")
        dlg.protocol("WM_DELETE_WINDOW", lambda:(_save_state2(),dlg.destroy()))
        if _saved2.get('path') and os.path.isfile(_saved2.get('path','')): _refresh()
    ...
    def _build_plc_tab(self, parent):
        ...
        # ── Sotto-notebook: Auto Export | DB Reader  (tab a sinistra)
        plc_nb = ttk.Notebook(parent)  # tab a sinistra rimossa (non supportata in Python 3.14+)
        ...
        plc_nb.select(0)  # apre Auto Export per prima
        # Monitor Real-Time rimosso in v5.0
        self._rt_running = False  # init per compatibilita'
    ...
    def _build_plc_autoexport(self, parent):
        ...
        # Variabili interne (usate da _autoexp_on_trigger e _autoexp_start)
        self._pv_autoexp_on        = tk.BooleanVar(value=True)
        self._pv_autoexp_viewer    = tk.BooleanVar(value=False)
        self._pv_autoexp_path      = tk.StringVar(value="")
        self._pv_autoexp_path_rej  = tk.StringVar(value="")

        ae_r1 = ttk.Frame(ae_frm); ae_r1.pack(fill="x", pady=2)
        ttk.Label(ae_r1, text="Trigger:").pack(side="left", padx=(0,2))
        ...
        # ── SQLite destinazione ───────────────────────────────────
        # Variabili SQL interne
        self._pv_autoexp_save_file = tk.BooleanVar(value=False)
        self._pv_autoexp_save_sql  = tk.BooleanVar(value=True)
        self._pv_autoexp_sql_path  = self._pv_global_sql_path

        sql_lf = ttk.LabelFrame(left, text="  SQLite  ", padding=4)
        sql_lf.pack(fill="x", padx=6, pady=2)
        sql_r2 = ttk.Frame(sql_lf); sql_r2.pack(fill="x", pady=2)
        ttk.Label(sql_r2, text="Archivio:", style="Muted.TLabel", width=9).pack(side="left")
        tk.Label(sql_r2, textvariable=self._pv_global_sql_path,
                 bg=DARK_BG, fg=MUTED_CLR, font=("Consolas",8),
                 anchor="w", wraplength=200).pack(side="left", fill="x", expand=True)
        ttk.Label(sql_lf, text="(da Impostazioni)",
                  style="Muted.TLabel", font=("Consolas",7)).pack(anchor="w")
    ...
    def _autoexp_start(self):
        ...
        if not self._autoexp_sql_open():
            self._plc_log_msg("\u2717 Impossibile aprire il database SQLite. Verificare le impostazioni.\n", "err")
            return
    ...
    def _build_sim_source_tab(self, parent):
        ...
        # Sorgente: solo SQLite (path globale dalla toolbar)
        self._pv_sim_source = tk.StringVar(value="sqlite")
        self._sim_src_file_frame = ttk.Frame(main)  # vuoto, per compat.
        self._pv_sim_src_file_info = tk.StringVar(value="")

        # Frame SQLITE
        self._sim_src_sql_frame = ttk.LabelFrame(main, text="  SQLite  ", padding=6)

        # Path database: usa path globale
        ...
        self._pv_sim_src_sql_path = self._pv_global_sql_path  # alias globale
        tk.Label(r1, textvariable=self._pv_sim_src_sql_path,
                 bg=DARK_BG, fg=MUTED_CLR, font=("Consolas",8),
                 anchor="w", wraplength=200).pack(side="left", padx=4, fill="x", expand=True)
    ...
    def _sim_src_browse_sql(self):
        p = filedialog.askopenfilename(parent=self, title="Database SQLite",
            filetypes=[("SQLite","*.sqlite *.db3"),("Tutti","*.*")])
        if p:
            self._pv_global_sql_path.set(p)  # *** v5.0 *** propaga a tutti
            self._settings['sqlite_path'] = p
            self._save_settings()
    ...
    def _build_sqlite_import_tab(self, parent):
        ...
        self._pv_sqlimp_dst = tk.StringVar(value=self._pv_global_sql_path.get() or os.path.join(os.path.expanduser("~"), "WeldExport", "weld_archive.sqlite"))
    ...
    def _build_sqlite_query_tab(self, parent):
        ...
        self._pv_sqlqry_path = self._pv_global_sql_path  # *** v5.0 *** alias global
    ...
    def _draw_polar_cylinder_profile(self, ax, angles_deg, samples, det_angle=None,
                                      weld_start=None, weld_end=None, color=None):
        """Vista polare del profilo REALE del cilindro (valori laser assoluti).
        Mostra la forma fisica del pezzo: eccentricita', ovalizzazione, ecc.
        Il raggio e' proporzionale al valore laser; normalizzato alla mediana."""
        if color is None: color = ACCENT
        ax.cla(); self._style_polar(ax)
        if len(angles_deg) < 3: return
        ar = np.radians(angles_deg)
        s  = np.array(samples, dtype=np.float32)
        med = float(np.nanmedian(s))
        if med < 1e-3: med = 1.0
        # Normalizza: 1.0 = mediana, amplifico le deviazioni per renderle visibili
        rng = float(np.nanmax(s) - np.nanmin(s))
        if rng < 1e-6: rng = 1.0
        # Raggio: 0.7 - 1.3 (centro=1.0 = mediana)
        rn = 1.0 + 0.5 * (s - med) / rng * 2
        rn = np.clip(rn, 0.05, 1.95)
        # Cerchio mediana
        ax.plot(ar, np.ones_like(ar), color=OK_CLR, linewidth=0.8,
                linestyle="--", alpha=0.4)
        # Profilo reale
        ax.fill(ar, rn, alpha=0.15, color=color)
        ax.plot(ar, rn, color=color, linewidth=1.2, alpha=0.85)
        # Marcatore detection
        if det_angle is not None:
            ax.axvline(np.radians(float(det_angle)), color=DET_CLR,
                       linewidth=2.0, alpha=0.85)
        # Zona saldatura
        if weld_start is not None and weld_end is not None:
            arc = np.radians(np.linspace(float(weld_start), float(weld_end), 50))
            ax.fill_between(arc, 0, 1.95, alpha=0.15, color=WELD_CLR)
        ax.set_xticks(np.radians([0,90,180,270]))
        ax.set_xticklabels(["0°","90°","180°","270°"], color=MUTED_CLR, fontsize=7)
        ax.set_title("Profilo reale cilindro", color=MUTED_CLR, fontsize=8, pad=8)
    ...
    def _draw_polar_tab(self):
        ...
        # Vista profilo reale cilindro
        da_val  = float(sc.get("rDetectedAtAngle", 0)) if sc.get("rDetectedAtAngle") else None
        ws_val  = float(sc.get("rWeldAngleStart", 0)) if sc.get("rWeldAngleStart") else None
        we_val  = float(sc.get("rWeldAngleEnd", 0)) if sc.get("rWeldAngleEnd") else None
        self._draw_polar_cylinder_profile(self.ax2b, d["angles"], d["samples"],
                                          det_angle=da_val, weld_start=ws_val, weld_end=we_val)
    ...
    def _build_stat_tab(self, parent):
        ...
        self._pv_stat_source = tk.StringVar(value="sqlite")  # sempre SQLite
        self._stat_files_frame = ttk.Frame(src_lf)  # vuoto, per compat. interna
        ...
        # Path file SQLite
        # Path SQLite: usa il path globale dalla toolbar (sempre visibile)
        sql_r1 = ttk.Frame(self._stat_sqlite_frame); sql_r1.pack(fill="x", pady=2)
        ttk.Label(sql_r1, text="SQLite:", style="Muted.TLabel").pack(side="left")
        tk.Label(sql_r1, textvariable=self._pv_global_sql_path,
                 bg=DARK_BG, fg=MUTED_CLR, font=("Consolas",8),
                 anchor="w", wraplength=200).pack(side="left", padx=4, fill="x", expand=True)
        self._pv_stat_sql_path = self._pv_global_sql_path  # alias al globale
        # Filtri rapidi
        ...
        tk.Label(self._stat_sqlite_frame, textvariable=self._pv_stat_sql_info,
                 bg=DARK_BG, fg=MUTED_CLR, font=("Consolas",8)).pack(anchor="w")
        self._stat_sqlite_frame.pack(fill="x")  # sempre visibile
        ...
        for lbl, key, dflt in [
            ...
            ("DetStart°", "detection_start_deg", "0.0"),  # *** v4.9 ***
        ]:
    ...
    def _stat_run(self):
        ...
        try:
            fixed = {
                ...
                "detection_start_deg": float(self._stat_fix["detection_start_deg"].get() or 0.0),  # *** v4.9 ***
            }
